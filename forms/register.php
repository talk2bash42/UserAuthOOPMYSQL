    ...
    <div class="all">
        ...
        <form class="form-control w-50" method="POST" action="../action.php">
            ...
            <div class="form-row flex justify-content-center">
                <div class="form-group">
                    ...
                    if (isset($_GET['name'])) {
                        $fullname = $_GET['name'];
                        echo ' <input type="text" class="form-control" id="fullnames" placeholder="Full Names" name="fullnames" required value="'.$fullname.'">';
                    } else
                    echo ' <input type="text" class="form-control" id="fullnames" placeholder="Full Names" name="fullnames" required>';
        ...
        if (isset($_GET['error'])) {
            ...
            if ($error == "WrongPasswordOrEmail") {
                echo "<p style='color: red; '> Wrong Password Or Email! </P>"; die();
            } elseif ($error == "userExist") {
                echo "<p style='color: red;  '> User Exist! </P>"; die();
            } elseif ($error == "PasswordTaken") {
                echo "<p style='color: red; '> Password Taken!</P>";  die();
            } elseif ($error == "WrongEmail") {
                echo "<p style='color: red; '> Wrong Email!</P>";  die();
            }
        }elseif (isset($_GET['update'])) {
            $update = $_GET['update'];
            if ($update == "UpdateSuccessful") {
                echo "<p style='color: green; '> Update Successful </P>";  die();
            } 
        }elseif (isset($_GET['login'])) {
            $login = $_GET['login'];
            if ($login == "Success") {
                echo "<p style='color: green; '> Login was Successful </P>";  die();
            } 
        }
        ?>
